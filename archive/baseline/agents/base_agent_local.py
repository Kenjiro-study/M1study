...
"""
# ローカルなhuggingfaceのモデルを使用するためのラッパークラス(2025/7/17 追加)
...
class LocalHuggingFaceModel(dspy.LM):
    def __init__(self, model_path: str, **kwargs):
        ...
        # 環境変数 HUGGING_FACE_HUB_TOKEN が自動で使われる
        # --- ここから修正 ---
        # 語彙ファイルへのフルパスを作成
        vocab_file_path = os.path.join(model_path, ".no_exist/0e9e39f249a16976918f6564b8830bc894c89659/tokenizer.model")
        
        # from_pretrained に vocab_file を直接渡す
        ...
        self.model = AutoModelForCausalLM.from_pretrained(
            model_path,
            torch_dtype=torch.bfloat16, # GPU利用を想定
            device_map="auto",
            **kwargs
        )
        self.history = []
    ...
    def basic_request(self, prompt, **kwargs):
        ...
        # 応答を生成
        outputs = self.model.generate(inputs, max_new_tokens=200, **kwargs)
...
class BaseAgent:
    ...
    def generate_response(self, action: str, price: Optional[float] = None) -> str:
        ...
        template = MODEL_CONFIGS[self.model_key].prompt_template
...
def test_base_agent():
    ...
    # --- ここを自作クラスに置き換え ---
    #model_path = os.path.join(agreemate_dir, "models/pretrained/models--meta-llama--Llama-3.1-8B-Instruct")
    # 自作したラッパークラスを初期化
    #test_lm = LocalHuggingFaceModel(model_path, trust_remote_code=True)
